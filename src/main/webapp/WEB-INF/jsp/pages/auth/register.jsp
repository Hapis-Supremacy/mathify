...
<style>
  *, *::before, *::after { box-sizing: border-box; }
  ...
  /* Checkbox */
  .auth-check { display:flex; align-items:flex-start; gap:10px; font-size:13px; color:var(--ink-2); line-height:1.45; cursor:pointer; user-select:none; }
  .auth-check input { position:absolute; opacity:0; pointer-events:none; }
  .auth-check__box { flex:none; width:20px; height:20px; margin-top:1px; border-radius:6px; border:1.5px solid var(--line); background:var(--paper); display:inline-flex; align-items:center; justify-content:center; color:#fff; transition:background .15s,border-color .15s; }
  .auth-check input:checked + .auth-check__box { background:var(--green); border-color:var(--green); }
  .auth-check__box svg { opacity:0; transition:opacity .12s; }
  .auth-check input:checked + .auth-check__box svg { opacity:1; }
  /* Strength meter */
  .auth-strength { display:flex; gap:5px; margin-top:2px; }
  .auth-strength span { flex:1; height:4px; border-radius:999px; background:var(--line); transition:background .2s; }
  @keyframes drift {
    ...
    const Register = () => {
      ...
      const handleSubmit = async (e) => {
        ...
        try {
          ...
          postTokenToServlet('/register', idToken);
        } catch (err) {
          ...
          <div className="auth-form-col">
            <div className="auth-card">
              ...
              <p className="auth-foot">
                ...
                <a href={PAGE_CTX + '/login'} className="auth-link">Sign in</a>
              </p>
            </div>
          </div>
        </div>
